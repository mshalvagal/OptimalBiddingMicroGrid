...
for k=1:NEpisodes
    linear_agent_params.alpha=linear_agent_params.alpha/1.005;
    linear_agent_params.epsilon=linear_agent_params.epsilon/1.005;
    if mod(k,500) == 0
%         agent_params.alpha = alpha_init - (alpha_init - alpha_final)*k/NEpisodes;
%         agent_params.epsilon = epsilon_init - (epsilon_init - epsilon_final)*k/NEpisodes;
    end
    ...
    actions2(k,:) = agent_sim2(i,bat_soc_init,env_params,linear_agent_params,energy_data);
...
%     bid_q_o = 5+(30-5).*rand(1,24);
    ...
    for j = 1:NBlocks
        ...
        agent_params.weights = ann_train(state(j,:),agent_params,reward(j),q_sa(j),qnext_sa(j),gamma);
    end
    ...
    performance_measures_agent2.reward(k,:) = performance.reward;
    ...
    for j = 1:NBlocks
        if j~=NBlocks
            [~,qnext_sa(j)] = greedy(demand_norm_a(i,j+1),solar_norm_a(i,j+1),bat_storage(k,j+1)/bat_cap,min_bid_q,max_bid_q, acp_a(i,j+1)/max_acp, (j+1)/NBlocks,linear_agent_params.weights);
        end
        present_state = basisExpansion(demand_norm_a(i,j), solar_norm_a(i,j), bat_storage(k,j)/bat_cap, actions(k,j)/max_bid_q, acp_a(i,j)/max_acp, j/NBlocks);
        q_sa(j)=valuefn(present_state,linear_agent_params.weights);
        linear_agent_params.weights = linear_agent_params.weights + linear_agent_params.alpha*(reward(j) + gamma*qnext_sa(j) - q_sa(j))*present_state';
    end
    
    if rem(k,agent_params.update_freq) == 0
        agent_params.target_weights = agent_params.weights;
    end
...
legend('Actual bill','Oracle bill','Oracle 2 bill','Bill without battery','Linear Agent bill');
