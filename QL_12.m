...
%     bat_eff2 = max(bat_eff_final,bat_eff_init - (bat_eff_init-bat_eff_final)*num_charge_cycles_la/bat_eff_lifetime);
...
%     bid_q_op = oracle_sim(i,bat_soc_init,'predicted',env_params,energy_data);
    ...
    [performance,bat_storage_la(k,:)] = evaluate_actions(i,bat_soc_init,bat_eff2,env_params,energy_data,actions_la(k,:));
...
%     totalbattBen2 = totalbattBen2 + sum(performance_measures_agent_la.bat_charge_cost(i, :));
...
Bill_diff = sum(perf_measures_agent.cost,2) - sum(perf_measures_agent_la.cost,2);
Bill_diff_total = sum(Bill_diff)

%% Effective cost per unit of electricity in a day
Bill_nn = sum(perf_measures_agent.cost,2);
Bill_la = sum(perf_measures_agent_la.cost,2);
for i=1:NEpisodes
    Cost_pu_nn(i) = Bill_nn(i)/consumption(i);
    Cost_pu_la(i) = Bill_la(i)/consumption(i);
end
Cost_sum1 = 0;
Cost_sum2 = 0;
for i=1:NEpisodes
    Cost_sum1 = Cost_sum1 + Cost_pu_nn(i);
    Cost_sum2 = Cost_sum2 + Cost_pu_la(i);
    Cost_pu_nn_avg(i) = Cost_sum1/i;
    Cost_pu_la_avg(i) = Cost_sum2/i;
end
figure()
plot(1:NEpisodes,Cost_pu_nn_avg)
hold on
plot(1:NEpisodes,Cost_pu_la_avg)
xlabel('Episodes','FontSize', 12);
ylabel('Daily per unit cost','FontSize', 12);
legend('NN Agent','Linear Agent');